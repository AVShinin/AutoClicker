...
namespace AutoClicker
{
    public class KeyboardHook : IDisposable
    {
        ...
        public delegate void LocalMouseEventHandler(MouseButtons button);
        public event LocalMouseEventHandler MouseKeyDown;
        public event LocalMouseEventHandler MouseKeyUp;

        public delegate void LocalMouseMoveEventHandler(int x, int y);
        public event LocalMouseMoveEventHandler MouseMoveDown;
        ...
        private int HookKBID = 0;
        private int HookMVID = 0;
        CallbackDelegate TheHookCB = null;
        CallbackDelegate TheHookMCB = null;
        ...
        public KeyboardHook(bool Global)
        {
            ...
            TheHookMCB = new CallbackDelegate(MouseHookProc);
            if (Global)
            {
                HookKBID = SetWindowsHookEx(HookType.WH_KEYBOARD_LL, TheHookCB,
                    0, //0 for local hook. eller hwnd til user32 for global
                    0); //0 for global hook. eller thread for hooken
                //HookMVID = SetWindowsHookEx(HookType.WH_MOUSE_LL, TheHookMCB,
                //    0, //0 for local hook. eller hwnd til user32 for global
                //    0); //0 for global hook. eller thread for hooken
            }
            else
            {
                HookKBID = SetWindowsHookEx(HookType.WH_KEYBOARD, TheHookCB,
        ...
        ~KeyboardHook()
        {
            if (!IsFinalized)
            {
                UnhookWindowsHookEx(HookKBID);
                UnhookWindowsHookEx(HookMVID);
                IsFinalized = true;
            }
        }
        public void Dispose()
        ...
        private int KeybHookProc(int Code, int W, int L)
        {
            if (Code < 0)
            {
                return CallNextHookEx(HookKBID, Code, W, L);
            }
            try
            {
                if (!Global)
        ...
        private int MouseHookProc(int Code, int W, int L)
        {
            if (Code < 0)
            {
                return CallNextHookEx(HookMVID, Code, W, L);
            }
            try
            {
