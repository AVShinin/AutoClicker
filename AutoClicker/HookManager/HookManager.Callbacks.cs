    ...
    public static partial class HookManager
    {
        ...
        private static int KeyboardHookProc(int nCode, Int32 wParam, IntPtr lParam)
        {
            ...
            if (nCode >= 0)
            {
                ...
                {
                    int stateCtrl = GetKeyState((int)Keys.ControlKey);
                    int stateShift = GetKeyState((int)Keys.ShiftKey);
                    int stateAlt = GetKeyState((int)Keys.Menu);

                    Keys keyData = (Keys)MyKeyboardHookStruct.VirtualKeyCode;

                    if(stateCtrl > 1 || stateCtrl < -1)
                        keyData = keyData | Keys.Control;

                    if (stateShift > 1 || stateShift < -1)
                        keyData = keyData | Keys.Shift;

                    if (stateAlt > 1 || stateAlt < -1)
